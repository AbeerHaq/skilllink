...
def add_issue_photos(booking_id: int, photo_path: str, description: str):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE bookings
        SET issue_photo_url = ?, issue_description = ?
        WHERE id = ?
    """, (photo_path, description, booking_id))
    conn.commit()
    conn.close()


def get_issue_photos(booking_id: int):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT issue_photo_url, issue_description FROM bookings WHERE id = ?",
        (booking_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
def create_booking(client_name: str, client_phone: str, provider_name: str, provider_phone: str,
                   ...
                   booking_type: str = "instant", scheduled_datetime: str = None, preferred_time_window: str = None):
    """
    ...
    Supports instant and scheduled bookings.
    """
    conn = get_db()
    cursor = conn.cursor()
    ...
    if booking_type == "scheduled":
        status = "Scheduled"
        step = 0
        date_display = f"Scheduled for {scheduled_datetime}"
    else:
        status = "In Progress"
        step = 0
    ...
    cursor.execute("""
        INSERT INTO bookings (booking_code, client_name, client_phone, provider_name, provider_phone,
                              ...
                              payment_method, otp_pin, status, booking_type, scheduled_datetime,
                              preferred_time_window, step, date_str, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (code, client_name, client_phone, provider_name, provider_phone, service_name,
          pickup, dropoff, fare, platform_fee, provider_payout,
          payment_method, otp_pin, status, booking_type, scheduled_datetime,
          preferred_time_window, step, date_display, now_str))
    ...
    notif_msg = (
        f"{provider_name} is scheduled for your {service_name} at {scheduled_datetime}."
        if booking_type == "scheduled"
        else f"{provider_name} is assigned to your {service_name}."
    )
    cursor.execute("""
        INSERT INTO notifications (user_phone, title, text, time, is_read)
        VALUES (?, ?, ?, ?, ?)
    """, (client_phone, "Booking Confirmed! 🚀", notif_msg, "Just now", 0))
